...
def test_station_mock_config():
    ...
    assert device.station_name == 'Superstation'

    module = device.modules[0]
    assert module.module_name == 'Livingroom'
    assert module.device.station_name == 'Superstation'
    assert module.measurements[0].data_type.name == 'Temperature'
    assert module.measurements[0].name == r'Superstation\Livingroom\Temperature'
    assert module.measurements[0].source_name == r'Superstation\Livingroom'

    module = device.modules[1]
...
def test_domain_login_mock():
    ...
    assert domain.devices[0].name == 'Superstation'
